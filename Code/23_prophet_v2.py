...
DATA_DIR = "../Data/Output/Prophetv2/"
OUTPUT_DIR = "../Output/prophetv2/"
...
def train_and_evaluate(train_path: str, test_path: str, model_name: str) -> float:
    ...
    plot_prediction(test_df, model_name)

    return rmse


def plot_prediction(df: pd.DataFrame, model_name: str):
    df = df.sort_values("ds")

    plt.figure(figsize=(12, 6))
    plt.plot(df["ds"], df["y"], label="Actual", color="blue", alpha=0.7)
    plt.plot(df["ds"], df["predicted"], label="Predicted", color="red", alpha=0.7)
    plt.xlabel("Date")
    plt.ylabel("Value")
